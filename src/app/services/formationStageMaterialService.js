...
async  function  getAllFormationStagesMaterials() {
	try {
    let  pool = await  sql.connect(dbSettings);
    let  allFormationStagesMaterials = await  pool.request().query("SELECT * from FormationStageMaterials");
    return  allFormationStageMaterials.recordsets;
  }
  catch (error) {
    console.log(error);
  }
